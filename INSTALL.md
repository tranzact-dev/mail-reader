# メール読み上げアプリ インストール手順

## 前提条件

- Windows 10 または Windows 11
- インターネット接続

## インストール（自動）

### 事前準備（開発者側）

以下のファイルをUSBメモリまたは共有フォルダにまとめておく:

```
セットアップ/
├── install.bat                ← リポジトリに同梱済み
├── python-3.13.5-amd64.exe   ← https://www.python.org/downloads/ から取得
├── Git-2.49.0-64-bit.exe     ← https://git-scm.com/downloads/win から取得
└── .env                       ← 設定済みのものを用意
```

`.env` の内容:
```
ANTHROPIC_API_KEY=（Claude APIキー）
BIGLOBE_EMAIL=（BIGLOBEメールアドレス）
BIGLOBE_PASSWORD=（BIGLOBEパスワード）
CASUAL_CONTACTS=
```

### 実行（ヘルパーさん）

1. USBメモリをPCに挿す
2. `install.bat` を右クリック →「管理者として実行」
3. 完了を待つ（5〜10分）

install.bat が自動で以下を実行します:
1. Python の有無を確認 → なければサイレントインストール
2. Git の有無を確認 → なければサイレントインストール
3. GitHubからアプリを `C:\mail-reader` に取得
4. 依存パッケージをインストール（pip install）
5. `.env` を配置
6. デスクトップに「メール読み上げ」ショートカットを作成

### 起動

デスクトップの「メール読み上げ」をダブルクリック。

## 本番モードへの切替

テスト完了後、`C:\mail-reader\mail_reader_gui.py` の29行目を変更:
```python
DRY_RUN = True   ← テスト中はこのまま
DRY_RUN = False  ← 本番運用時に変更（メール送信・既読マークが有効になる）
```

## トラブルシューティング

### 「python が見つかりません」と表示される

→ install.bat を管理者として再実行してください。

### 「ModuleNotFoundError: No module named 'xxx'」と表示される

→ 依存パッケージが不足しています。コマンドプロンプトで以下を実行:
```
cd C:\mail-reader
pip install -r requirements.txt
```

### 「未読メールはありません」と表示されるが実際には未読がある

→ `C:\mail-reader\.env` のメールアドレス・パスワードを確認してください。
→ 直近30日間の未読メールのみが対象です。

### 音声が出ない

→ PCの音量を確認してください。
→ Edge-TTS はインターネット接続が必要です。

### 返信が生成されない（「返信を考えています」のまま止まる）

→ `.env` の ANTHROPIC_API_KEY を確認してください。
→ APIクレジット残高: https://console.anthropic.com/

## アップデート

自動です。デスクトップのショートカットから起動するたびに、最新版を自動取得します。

依存パッケージが追加された場合のみ、手動で以下を実行:
```
cd C:\mail-reader
pip install -r requirements.txt
```
